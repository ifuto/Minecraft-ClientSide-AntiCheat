...
public class SelfBanPacketSender {
    ...
    public void sendHandshake() {
        try {
            ...
            json.addProperty("playerUuid", playerUuid);
            json.addProperty("playerName", playerName);
            json.addProperty("modVersion", "2.0.0-evolved");
            json.addProperty("os", System.getProperty("os.name"));
            json.addProperty("javaVersion", System.getProperty("java.version"));
            // HWID
            String hwid = "unknown";
            try {
                hwid = new com.anticheat.client.detection.HardwareFingerprint().getHwid();
            ...
            if (ClientPlayNetworking.canSend(HANDSHAKE_CHANNEL)) {
                ...
                System.out.println("[Anticheat] Handshake sent with HWID " + hwid);
            }

            // Also send HWID separately for HWID ban manager
            sendHwid(hwid);
    ...
    public void sendHwid(String hwid) {
        try {
            ...
            long ts = System.currentTimeMillis() / 1000;
            json.addProperty("ts", ts);
            String dataToSign = playerUuid + "|HWID|" + hwid + "|" + ts;
            String sig = ViolationReporter.HmacUtil.hmacSha256(config.hmacSecret, dataToSign);
            json.addProperty("sig", sig);

            PacketByteBuf buf = PacketByteBufs.create();
            buf.writeString(json.toString());
            if (ClientPlayNetworking.canSend(HWID_CHANNEL)) {
                ClientPlayNetworking.send(HWID_CHANNEL, buf);
            }
    ...
    public void sendChallengeResponse(String challenge, String response) {
        try {
            ...
            long ts = System.currentTimeMillis()/1000;
            json.addProperty("ts", ts);
            // No HMAC for challenge response? Use same
            String dataToSign = playerUuid + "|CHALLENGE|" + challenge;
            json.addProperty("sig", ViolationReporter.HmacUtil.hmacSha256(config.hmacSecret, dataToSign));
            PacketByteBuf buf = PacketByteBufs.create();
            buf.writeString(json.toString());
            if (ClientPlayNetworking.canSend(CHALLENGE_CHANNEL)) {
                ClientPlayNetworking.send(CHALLENGE_CHANNEL, buf);
                System.out.println("[Anticheat] Challenge response sent: " + response);
            }
        } catch (Exception e) { e.printStackTrace(); }
    }
