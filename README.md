...
| 入れてもいない禁止MODがあると言われてキックされる | **サーバー 1.0.32 以前の既知問題**: HMAC 鍵が一致しないだけで `UNVERIFIED` が critical 扱いになり、「許可されていないMOD/パックが検出されました」でキックされていた。①クライアント jar と**同じ run の成果物**の `mcsa-client-<ver>-hmac-key.txt` を `hmac.key` に貼って `/ac reload`（これで直る）。②根本対策は**サーバー 1.0.33 以降**（`UNVERIFIED`/`CLIENT_TAMPERED` はキック対象外になり、コンソールに対処法が出る。キック時はフラグ名も併記）。`/ac policy pin-client` で古い jar を固定したままの場合も同様（`CLIENT_TAMPERED`） |
| レポートは届くが `UNVERIFIED` ★ が付く | `hmac.key` がクライアントのビルドと不一致。jar と `mcsa-client-<ver>-hmac-key.txt` を**同じ run の成果物から**取り、`config.yml` に反映して `/ac reload`。Secrets に `MCSA_HMAC_SEED` を設定すれば全ビルドで同一鍵になり不一致自体が消える（ci/ のワークフロー再コピーが必要）。build ≤29 の Multi Build run は鍵ファイルが成果物に入っていないので、同梱 `build-log.txt` の `[MCSA] HMAC 鍵` 行の値を使う。サーバー 1.0.33 以降なら接続時とレポート受信時に**鍵の指紋（keyId）つきで**コンソール警告が出る |
